...
from sandbox import run_code_on_inputs
...
def read_inputs_and_prepare(cfg) -> pd.DataFrame:
    ...
    if cfg.reference_file_path is not None:
        ...
        print(f"Unique inputs in reference: {len(ref_df['slower_program'].unique())}")
...
def write_programs_read_ground_truth(
    ...
    for _, row in tqdm(merged.iterrows(), total=len(merged), desc="writing programs"):
        ...
        with open(
            os.path.join(output_code_location, f"{submission_id_v0}_{problem_id}_slow.{lang_file_ending}"), "w", encoding='utf-8'
        ) as f:
        ...
        with open(
            os.path.join(output_code_location, f"{submission_id_v0}_{problem_id}_reference.{lang_file_ending}"), "w", encoding='utf-8'
        ) as f:
    ...
    for _, row in tqdm(merged.iterrows(), total=len(merged), desc="running programs"):
        ...
        try:
            ...
            for (tag, suffix) in tag_to_path:
                ...
                avg_time, std_time, avg_acc, stats = run_code_on_inputs(  # type: ignore
                ...
                print('After running acc: ', avg_acc)
