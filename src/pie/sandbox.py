...
def run_python_code_on_inputs(
    ...
    return_if_acc_below: float = None,  # type: ignore | if the accuracy is below this value, then return
    ...
    for test_case_idx in range(num_test_cases):
        ...
        for trial_idx in range(num_runs_per_test_case):
            try:
                ...
                _per_trial_times.append(time_taken * 1000)
                if output is None:
                ...
                if trial_idx >= ignore_first_k:
                    ...
                    if ground_truths is not None:
                        ...
                        if return_if_acc_below is not None and accuracy < return_if_acc_below:
                            ...
                            return (time_taken, 0, accuracy, 0)
    ...
    for i in range(num_runs_per_test_case):
        add = 0
        for j in range(len(per_trial_times)):
            add += per_trial_times[j][i]
        stats.append(add)
    ...
    if return_dict:
        return {
            ...
            #"code_stats": stats_np,
            #"code_stats_all": stats_np * 1000
        }
    else:
        return np.mean(times_millisec), np.std(times_millisec), np.mean(accs), stats_np  # type: ignore
...
def test_cpp_reference(number_to_test: int, path_to_ref: str, report_dir: str, test_case_path: str) -> None: 
    ...
    for i, ref in enumerate(refs):
        ...
        pbar.update(1)
        pbar.set_description(f"Compiled {meta_results_dict['slow_compiled'] + meta_results_dict['fast_compiled']}/{(i+1)*2}, Ran {meta_results_dict['slow_ran'] + meta_results_dict['fast_ran']}/{(i+1)*2}, Fast is faster {meta_results_dict['fast_is_faster']}/{i+1}")
    pbar.close()
